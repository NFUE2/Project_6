...
public class P_TentaclypseDispenserAttack : MonoBehaviourPun, IAttackPattern
{
    ...
    private IEnumerator DispenserAttack(Vector3 randPosLeft, Vector3 randPosRight)
    {
        ...
        var dispenserLeft = PhotonNetwork.Instantiate("Prototype/" + dispenser.name,Vector2.zero,Quaternion.identity);
    ...
    private IEnumerator AllRoundAttackCoroutine(Vector3 randPosLeft, Vector3 randPosRight)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            for (int j = 0; j < bulletCount; j++)
            {
                ...
                GameObject go1 = PhotonNetwork.Instantiate("Prototype/" + bullet.name, dispenser.transform.position, Quaternion.identity);
